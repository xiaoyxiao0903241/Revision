...
const BondList = [
  ...
  {
    name: 'DAI_Bond_(30',
    address: stableBond30,
    color: 'text-orange-500',
  },
  {
    name: 'DAI_Bond_(90',
    address: stableBond90,
    color: 'text-orange-500',
  },
  {
    name: 'DAI_Bond_(180',
    address: stableBond180,
    color: 'text-orange-500',
  },
  {
    name: 'DAI_Bond_(360',
    address: stableBond360,
    color: 'text-orange-500',
  },
];
...
export const getMyBondList = async ({ address }: { address: Address }) => {
  const calls = [
    {
      address: stableBond5 as `0x${string}`,
      abi: BondAbi as Abi,
  ...
  try {
    ...
    if (res.length && res[0].data) {
      return Number(res[0].data as unknown as bigint);
    }
    return 0;
  } catch (error) {
    ...
    const calls = [
      {
        ...
        args: [TOKEN_ADDRESSES.DAI, parseUnits(amount, 18).toString()],
      },
    ];
    const res = await executeMulticall<[bigint]>({
      calls,
