...
import BondAbi from '@/src/constants/BondAbi.json';
import LPHelperAbi from '@/src/constants/LPHelper.json';
import ITreasuryAbi from '@/src/constants/ITreasury.json';
...
import PANCAKESWAP_ROUTER_ABI from '@/src/constants/RouterAbi.json';
const BondList = [
  {
    name: 'LP_Bond_(5',
    address: liquidityBond5,
    color: 'text-orange-500',
  },
  {
    name: 'LP_Bond_(30',
    address: liquidityBond30,
    color: 'text-orange-500',
  },
  {
    name: 'LP_Bond_(90',
  ...
  {
    name: 'LP_Bond_(180',
    address: liquidityBond180,
    color: 'text-orange-500',
  },
  {
    name: 'LP_Bond_(360',
    address: liquidityBond360,
    color: 'text-orange-500',
  },
];
      ...
      BondList.forEach((it, index) => {
        ...
        list.push({
          ...
          bondPrice: res[3 * index + 1].success
            ? formatUnits(res[3 * index + 1].data as bigint, 18)
            : '0',
          roi: res[3 * index + 1].success
            ? Number(
                ((Number(formatUnits(priceData[1], 18)) -
                  Number(formatUnits(res[3 * index + 1].data as bigint, 18))) /
                  Number(formatUnits(res[3 * index + 1].data as bigint, 18))) *
                  100
              ).toFixed(4)
            : 0,
...
export const getMyBondList = async ({ address }: { address: Address }) => {
  const calls = [
    {
      address: liquidityBond5 as `0x${string}`,
      abi: BondAbi as Abi,
      functionName: 'getBondCount',
      args: [address],
    },
    {
      address: liquidityBond30 as `0x${string}`,
      abi: BondAbi as Abi,
      functionName: 'getBondCount',
      args: [address],
    },
...
// 预估获取 lp 数量
export const getLpEstimate = async ({ amount }: { amount: string }) => {
  const calls = [
    {
      address: LPHelper as `0x${string}`,
      abi: LPHelperAbi as Abi,
      functionName: 'estimateLPInfo',
  ...
  if (res.length && res[0].data) {
    return formatUnits(res[0].data[0] as unknown as bigint, 18);
  }
  return 0;
};
    ...
    const calls = [
      {
        ...
        args: [LP_Token, parseUnits(amount, 18).toString()],
      },
    ];
    const res = await executeMulticall<[bigint]>({
      calls,
