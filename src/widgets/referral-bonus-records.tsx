...
import { Button, Card, CardContent, Pager, Tabs } from "~/components"
...
const records = Array.from({ length: 4 }).map((_, index) => ({
  id: index,
  event: "Bonus",
  sourceAddress: "OX1ert...f9g",
  sourceAddressTransactionAmount: 28.66,
  bonusAmount: 1.05,
  transactionHash: "OX1ert...f9g",
...
export const ReferralBonusRecords: FC = () => {
              ...
              {filteredRecords?.length > 0 ? (
                filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <Cell title={t("source_address")}>
                      {record.sourceAddress}
                    </Cell>
                    <Cell title={t("source_address_transaction_amount")}>
                      {formatDecimal(record.sourceAddressTransactionAmount, 2)}{" "}
                      OLY
                    </Cell>
                    <Cell title={t("bonus_amount")}>
                      {formatDecimal(record.bonusAmount, 2)} OLY
                    </Cell>
                    <Cell title={t("transaction_hash")}>
