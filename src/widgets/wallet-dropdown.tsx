...
import { Button, Icon, IconFontName, Segments, View } from '~/components';
...
export const useWalletDropdownStore = createWithEqualityFn<{
  recieveDialogOpen: boolean;
  walletDialogOpen: boolean;

  copeA: number;
  copeB: number;
}>(() => ({
  recieveDialogOpen: false,
    ...
    try {
      if (navigator?.clipboard?.writeText) {
        ...
        useWalletDropdownStore.setState({
          copeB: useWalletDropdownStore.getState().copeB + 1,
        });
        return;
      }
...
const RecieveDialog: FC = () => {
  const t = useTranslations('common');
  const { address } = useAccount();
  const { recieveDialogOpen } = useWalletDropdownStore();
  console.log('测是', recieveDialogOpen);
  return (
    <ResponsiveDialog
      open={recieveDialogOpen}
      onOpenChange={recieveDialogOpen =>
        useWalletDropdownStore.setState({ recieveDialogOpen })
      }
      title={t('receive')}
    >
      <div className='flex flex-col items-center justify-center'>
        ...
        <Image src='/images/icon/fox.png' alt='wallet' width={24} height={24} />
...
const WalletDialog: FC<{
  ...
  return (
    <>
      {/* Header */}
      <div className='flex flex-col md:p-6 gap-6 transition-all'>
      ...
      <div className='md:px-6 pb-6'>
                ...
                >
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <div className='text-xs text-white/50'>{asset.chain}</div>
                    </div>
                  </div>
        ...
        {activeTab === 'account' && (
          <div className='space-y-6'>
            ...
            {/* Reward Section */}
            <div className='space-y-3'>
              <span className='text-white/50 text-xs px-0'>{t('reward')}</span>
              <div className='space-y-2'>
...
export function WalletDropdown({
  ...
  const { walletDialogOpen, ...rest } = useWalletDropdownStore();
  const { isMobile } = useResponsive();
  console.log('当前', isMobile);
  useUnmount(() => {
    useWalletDropdownStore.setState({
      recieveDialogOpen: false,
      walletDialogOpen: false,
    ...
    <>
      {isMobile ? (
              ...
              {walletDialogOpen && (
                <Portal>
                  ...
                  <motion.div
                    ...
                    className={cn('fixed left-1 bottom-4 right-1 z-50')}
                  >
                    <View
                      clipDirection='topLeft-bottomRight'
                    ...
                    >
                      <WalletDialog handleDisconnect={handleDisconnect} />
                    </View>
                  </motion.div>
                </Portal>
              )}
        ...
        <DropdownMenu
          ...
          onOpenChange={walletDialogOpen => {
            useWalletDropdownStore.setState({ walletDialogOpen });
          }}
        >
